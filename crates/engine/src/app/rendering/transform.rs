...
pub fn camera_pixels_per_world(camera: &Camera2D) -> f32 {
    PIXELS_PER_WORLD * camera.effective_zoom()
}
...
mod tests {
    ...
    fn world_to_screen_then_back_is_consistent() {
        for zoom in [0.5_f32, 1.0_f32, 2.0_f32] {
            let camera = Camera2D {
            ...
            let tolerance = 0.5 / camera_pixels_per_world(&camera);
            assert!((output.x - input.x).abs() <= tolerance);
            assert!((output.y - input.y).abs() <= tolerance);
        }
    }
}
